...
export default{
	...
	data(){
        ...
        let len = serverUrl.length;
        if (len && serverUrl.substr(len - 1, 1) == "/") {
            serverUrl = serverUrl[0, len -1];
        }
		return {
            urlPrefix:serverUrl,//下载前缀
            dialogImageUrl: '',
            dialogVisible: false,
            disabled: false,
			fileInnerList:[],
    ...
    created(){
        ...
        let fileInnerList = this.files &&this.files.map(item=>{
            item['url'] = this.urlPrefix+'/hyperone/file/download/'+item.id;
            return item;
        }) ||[];
